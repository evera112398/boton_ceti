...
class ChangePasswordInApp extends StatefulWidget {
  const ChangePasswordInApp({
    super.key,
  });

  @override
  State<ChangePasswordInApp> createState() => _ChangePasswordInAppState();
}

class _ChangePasswordInAppState extends State<ChangePasswordInApp>
  ...
  void initState() {
    ...
    passwordFocusNodes[2].addListener(() {
      if (!passwordFocusNodes[2].hasFocus &&
  ...
  void _checkPassword(String password) {
    ...
    if (passwordController[1].text.length >= 8) {
      _is8Characters = true;
    }
    if (passwordController[2].text.isNotEmpty) {
      if (passwordController[2].value == passwordController[1].value) {
  ...
  void changePassword() async {
    ...
    final updatePasswordFuture =
        Future.delayed(const Duration(seconds: 2)).then(
      (value) => singletonProvider.usuariosController.updatePassword(
        passwordController[0].text,
        passwordController[1].text,
        passwordController[2].text,
      ),
    );
    await showDialog(
      context: context,
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => DynamicAlertDialog(
          actions: hasError
              ? [
                  ElevatedButton(
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      children: [
                                        ...
                                        const SizedBox(height: 20),
                                        TextInput(
                                          focusNode: passwordFocusNodes[2],
                                          isPassword: true,
                                          controller: passwordController[2],
                                          autofillHints: const [
                                            AutofillHints.name
                                          ],
